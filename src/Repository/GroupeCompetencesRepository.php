...
class GroupeCompetencesRepository extends ServiceEntityRepository
{
    ...
    public function findCompetence($idGpreCompetence,$idCompetence){
        return $this->createQueryBuilder('g')
                    ->innerJoin('g.competences','c')
                    ->andWhere("g.id = :idGrpeCompetence")
                    ->andWhere('c.id = :idcompetence')
                    ->setParameter('idCompetence',$idCompetence)
                    ->setParameter('idGrpeCompetence',$idGpreCompetence)
                    ->getQuery()
                    ->getResult();
    }
