...
if (isset($content['description']) && !empty($content['description'])
    && isset($content['dateCreation']) && !empty($content['dateCreation'])
    && isset($content['idApprenant']) && !empty($content['idApprenant'])) {
  ...
  //TODO: Recuperation du fil de discussion du livrable partiel
   ...
   $fildiscussion=$filDiscussionRepository->find( $livApprenants->getId());
                  // dd($fildiscussion);
    }
}
   // TODO: creation et enregistrement du commentaitre
     ...
     return $this->json("votre commentaire a été enregistré avec successsss", 201);
     }
     return $this->json("echec d'enregistrement",404);
   }
