...
class ProfilsortieController extends AbstractController
{
    ...
    {
        $promo = $promoRepository->findOneBy(["id" => $id_promo]);if($promo){
            $profils = $profilsortieRepository->findOneBy(["id" => $id_promo]);
}
        if ($profils) {
        ...
        //       // dd($promos); //  il maffiche la promo et le groupe principal
        //         }
        //     foreach($promos as $groupe) {
        //        if($groupe->getApprenant() ){ 
        //         return $this->json($profils, 200, [], ["groups" => ["promoProfilsortie:read"]]);
        //        }
        //    }
        // }
