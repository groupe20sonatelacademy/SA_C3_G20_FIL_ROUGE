...
class ProfilsortieController extends AbstractController
{
...

    {
        ...
        $profils = $profilsortieRepository->find($id_profilsortie);
        ...
        if ($promo) {
            ...
            foreach ($groupe as  $groupes) {
                ...
                // return $this->json($apprenants);il maffiche les apprenants
                foreach ($apprenants as $value) {
                    if ($value->getProfilsortie()) {
                        $maProfil = $value->getProfilsortie();
                       // return $this->json($maProfil);il maffiche les profilsde sortie
                        
                         if ($maProfil == $profils){
                            $TabProfil[]=$maProfil->getApprenant();
                                return $this->json($TabProfil, 200);
                         }
                    }
                }
            }
        }
        return $this->json(["message" => " Cette ressource n'existe pas", 404]);
    }
                ...
                if ($promo) {
                 ...
                 //dd($apprenants);   il maffiche la promo
                  ...
                  foreach($apprenants as  $profils) {
                    ...
                    return $this->json($groupe > getProfilsortie(), 200, [], ["groups" => ["profilsortieApp:read"]]);
                      }
                  }
