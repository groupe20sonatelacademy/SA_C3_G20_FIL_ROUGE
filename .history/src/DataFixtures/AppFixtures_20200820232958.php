...
$tabLivrableAttendu=["github","figma","trello","deploiement"];
foreach ($tabLivrableAttendu as $libelle){
   $livattendu= new LivrableAttendus();
   $livattendu->setLibelle($libelle);
   $manager->persist($livattendu);
}
        $ressource= new Ressource();
        $ressource->setTitre("projet fil rouge")
                   ->setUrl("www.yayefall.sn")
                   ->setPieceJointe('document');
        ...
        $brief->setLangue($faker->randomElement(['francais','anglais','chinois','arabe']))
               ...
               ->setContexte('en formation')
               ->setCriterPerformance('avor un bon niveau en infomatique')
               ->setDescription('jaime pas les beugg')
               ->setModalitePedagogique('il faut avoir de la pedagogique')
               ->setPhoto('default.jpeg')
               ->setStatut('valide')
               ->setDateCreation($faker->dateTime())
               ->setDateDecheance($faker->dateTime())
               ->setFormateur($user)
               ->addTag($tag)
               ->addNiveau($niveau)
               ->addLivrableAttendu($livattendu)
               ->setReferentie($referentiel)
                ->addRssource($ressource)
               ->setArchivage(0);
